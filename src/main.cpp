...
#if defined(NDEBUG)
#error "CORONAatHOME cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zCORO spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zCORO spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zCORO spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        nSubsidy = 2120000 * COIN;
    } else if (nHeight < 172800 && nHeight > 0) {
        nSubsidy = 0.10 * COIN;
        ...
        nSubsidy = 0.16 * COIN;
    } else if (nHeight < 864000 && nHeight >= 691200) {
        nSubsidy = 0.18 * COIN;
    } else if (nHeight < 1036800 && nHeight >= 864000) {
        nSubsidy = 0.20 * COIN;
    } else if (nHeight < 1209600 && nHeight >= 1036800) {
        nSubsidy = 0.22 * COIN;
    } else if (nHeight < 1382400 && nHeight >= 1209600) {
        nSubsidy = 0.24 * COIN;
    } else if (nHeight < 1555200 && nHeight >= 1382400) {
        nSubsidy = 0.25 * COIN;     
    } else if (nHeight < 1814400 && nHeight >= 1555200) {
        nSubsidy = 0.28 * COIN;
    } else if (nHeight < 2073600 && nHeight >= 1814400) {
        nSubsidy = 0.32 * COIN;
    } else if (nHeight < 2332800 && nHeight >= 2073600) {
        nSubsidy = 0.38 * COIN;     
    } else if (nHeight < 2592000 && nHeight >= 2332800) {
        nSubsidy = 0.48 * COIN;     
    } else if (nHeight < 3110400 && nHeight >= 2592000) {
        nSubsidy = 0.60 * COIN;
    } else if (nHeight < 3628800 && nHeight >= 3110400) {
        nSubsidy = 0.76 * COIN;
    } else if (nHeight < 4147200 && nHeight >= 3628800) {
        nSubsidy = 1 * COIN;  
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from CORONAatHOME
...
{
    RenameThread("coronaathome-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZCOROMinted()
{
...
void RecalculateZCOROSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zCORO supply
...
bool RecalculateCOROSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zCORO with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zCOROSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert CORO to zCORO
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zCORO spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zCORO serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zCORO mints and %d zCORO spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // CORONAatHOME: We use certain sporks during IBD, so check to see if they are
