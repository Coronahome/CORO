...
#define URI_SCHEME "coronaathome"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a CORONAatHOME address (e.g. %1)").arg("Co7ZBV75S51JBUponYd4nLbSERN8dauYeT"));
#endif
...
{
    // return if URI is not valid or is no CORONAatHOME: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::CORO, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert coronaathome:// to coronaathome:
    //
    //    Cannot handle this later, because coronaathome:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::CORO, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open coronaathome.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "CORONAatHOME.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for CORONAatHOME.lnk
...
{
    return GetAutostartDir() / "coronaathome.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a coronaathome.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=CORONAatHOME\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the coronaathome app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add coronaathome app to startup item list
