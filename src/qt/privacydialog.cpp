                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zCORO ought to be enough for anybody." - Bill Gates, 2017
    ui->zCOROpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzCOROSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzCORO->setEnabled(false);
        ui->pushButtonMintzCORO->setToolTip(tr("zCORO is currently disabled due to maintenance."));

        ui->pushButtonSpendzCORO->setEnabled(false);
        ui->pushButtonSpendzCORO->setToolTip(tr("zCORO is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zCOROpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzCORO_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCORO is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zCORO...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zCORO in ") +
...
void PrivacyDialog::on_pushButtonSpendzCORO_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCORO is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zCORO
        sendzCORO();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zCORO
    sendzCORO();
}

void PrivacyDialog::on_pushButtonZCOROControl_clicked()
{
    ZCOROControlDialog* zCOROControl = new ZCOROControlDialog(this);
    zCOROControl->setModel(walletModel);
    zCOROControl->exec();
}

void PrivacyDialog::setZCOROControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzCOROSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzCORO()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid CORONAatHOME Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zCOROpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zCORO is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " CORO </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zCORO</b>";
    ...
    // use mints from zCORO selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZCOROControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZCOROControlDialog::GetSelectedMints();
    }
    ...
    // Clear zCORO selector in case it was used
    ZCOROControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zCORO Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " CORO, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zCORO Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zCORO </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zCORO "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zCORO "));
    ui->labelzCOROAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzCOROSyncStatus->setVisible(fShow);
}
