...
#include "ui_zCOROcontroldialog.h"
...
std::list<std::string> ZCOROControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZCOROControlDialog::listMints;

ZCOROControlDialog::ZCOROControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZCOROControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZCOROControlDialog::~ZCOROControlDialog()
{
    delete ui;
}

void ZCOROControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCOROControlDialog::updateList()
{
...
void ZCOROControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCOROControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZCORO_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZCOROControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZCOROControlDialog::GetSelectedMints()
{
...
void ZCOROControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
